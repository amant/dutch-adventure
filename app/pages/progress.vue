...
<template>
  <section class="progress-view">
    ...
    <p class="muted">
      Based on your practice across all capabilities.
    </p>

    <div
      v-if="canDoItems.length > 0"
      class="card can-do-card"
    >
      <div class="eyebrow">
        Capabilities
      </div>
    ...
    <div class="pipeline-section">
      ...
      <div class="pipeline-container card">
        <div class="pipeline-grid">
          ...
          <div class="pipeline-arrow active">
            →
          </div>
    ...
    <div class="fluency-section">
      <div class="trend-grid">
        <div class="card fluency-card">
          ...
          <div class="stat-desc">
            <p v-if="averageRetrievalSpeed > 5">
              You are in the <strong>Thinking</strong> phase. Retrieval is conscious and slow.
            </p>
            <p v-else-if="averageRetrievalSpeed > 2">
              You are reaching <strong>Functional Fluency</strong>. Retrieval is becoming semi-automatic.
            </p>
            <p v-else-if="averageRetrievalSpeed > 0">
              You are reaching <strong>Automaticity</strong>. You retrieve Dutch almost as fast as your native language.
            </p>
            <p v-else>
              Start missions and drills to measure your retrieval speed.
            </p>
          </div>
        </div>

        <div
          v-if="naturalnessTrend"
        ...
        >
          ...
          <div class="trend-meta">
            <div
              ...
              :class="{ up: naturalnessTrend.change > 0, down: naturalnessTrend.change < 0 }"
            >
              {{ naturalnessTrend.change > 0 ? '↑' : naturalnessTrend.change < 0 ? '↓' : '→' }}
              {{ Math.abs(naturalnessTrend.change) }}%
            </div>
            <p class="muted">
              Trend over last 10 interactions.
            </p>
            <p class="desc">
              B2 learners should aim for >80% naturalness using native particles and flow.
            </p>
          </div>
        </div>
      </div>
      ...
      <div
        v-if="retrievalTrend"
        class="card speed-trend-card mt-6"
      >
    ...
    <div
      v-if="recentGains.length > 0"
      class="recent-progress"
    >
      <h2>Recent Gains</h2>
      <div class="grid">
        <div
          v-for="gain in recentGains"
          :key="gain.label"
          class="card gain-card"
        >
