...
<template>
  <div class="fixed-prep-drill">
    <div class="card drill-card">
      <div class="header">
        <div class="eyebrow">B2 Fixed Prepositions &amp; Prepositional Regimes</div>
        <div class="badge" :class="exercise.fixedPrepositionData?.collocationType">
          {{ typeBadge }}
        </div>
      </div>
      ...
      <div class="drill-blueprint mt-6">
        ...
        <div class="source-box context-box">
          <div class="box-label">Context / Situatie</div>
          <p class="source-text">{{ exercise.fixedPrepositionData?.contextPrompt }}</p>
          <p v-if="exercise.fixedPrepositionData?.meaningContext" class="meaning-note">
            💡 <em>Betekenis: {{ exercise.fixedPrepositionData.meaningContext }}</em>
          </p>
        </div>

        <!-- Transfer Error Warning -->
        <div v-if="exercise.fixedPrepositionData?.commonTransferErrors?.length" class="transfer-warning-box">
          <div class="warning-title">⚠️ Let op: Vermijd taaltransferfouten (Do not use)</div>
        ...
        <div v-if="exercise.fixedPrepositionData?.structureFormula" class="formula-blueprint-container">
        ...
        <div v-if="exercise.fixedPrepositionData?.hint" class="hint-container">
      ...
      <form class="production-form" @submit.prevent="handleSubmit">
        <label class="form-label" for="prep-input">
