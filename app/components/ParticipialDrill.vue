...
const typeBadge = computed(() => {
  switch (props.exercise.participialData?.triggerType) {
    case 'gerundive-modal': return 'Modaal Deelwoord / Gerundivum (Te + Infinitief)'
    case 'present-participle-attr': return 'Attributief Tegenwoordig Deelwoord (Inf + d + e)'
    case 'past-participle-attr': return 'Attributief Voltooid Deelwoord (Voltooid Deelwoord + e)'
    case 'al-participle-simultaneous': return 'Gelijktijdigheid met "Al" (Al + Tegenwoordig Deelwoord)'
    case 'concise-clause': return 'Beknopte Deelwoordconstructie (Gelet op / Gezien / Aangekomen)'
    default: return 'Deelwoordconstructies & Gerundivum'
  }
})
</script>

<template>
  <div class="participial-drill">
    <div class="card drill-card">
      <div class="header">
        <div class="eyebrow">B2 Advanced Syntax & Participial Mastery</div>
      ...
      <div class="instruction">
        ...
        <p class="muted">
          Transform the full clause or descriptive sentence into a concise, professional Dutch participial construction.
          Pay close attention to gerundive positioning with <em>te</em>, inflection endings (<em>-e</em>), and separable verb prefixes.
        </p>
      </div>

      <div class="drill-sources-view mt-6">
        <div class="source-box base-clause-box">
          <div class="box-label">Uitgangszin / Beschrijving (Base Clause / Description)</div>
          <p class="source-text">{{ exercise.participialData?.baseClause }}</p>
        </div>

        <div v-if="exercise.participialData?.structureFormula || exercise.participialData?.participleCue" class="formula-blueprint-container">
          <div class="blueprint-label">Constructieschema (Structural Blueprint):</div>
          <div class="blueprint-pills">
            <span v-if="exercise.participialData?.participleCue" class="pill cue">
              Kerndeelwoord / Cue: <strong>{{ exercise.participialData.participleCue }}</strong>
            </span>
            <span v-if="exercise.participialData?.structureFormula" class="pill formula">
              Formule: <code>{{ exercise.participialData.structureFormula }}</code>
            </span>
          </div>
        </div>
        ...
        <div class="source-box combined-target">
          <div class="box-label">Jouw Deelwoordconstructie (Your Participial Phrase / Sentence)</div>
          <div class="input-wrapper">
            <textarea
              ref="textarea"
              v-model="answer"
              class="drill-input"
              placeholder="Typ hier de beknopte deelwoordconstructie..."
              :disabled="!!feedback"
              @keydown.enter.prevent="handleSubmit"
            ></textarea>
            <div v-if="exercise.participialData?.hint" class="hint-text">
              Hint: <span>{{ exercise.participialData.hint }}</span>
            </div>
          </div>
        </div>
      </div>
...
<style scoped>
.participial-drill { max-width: 650px; margin: 0 auto; }
...
.source-box.base-clause-box { background: #f8fafc; }
.source-box.combined-target { border-color: #0d9488; background: white; box-shadow: 0 4px 12px rgba(13, 148, 136, 0.06); }
...
.pill strong { color: #0d9488; }
.pill.formula code { font-family: monospace; font-size: 12px; color: #0f766e; font-weight: 600; }
...
.mini-lesson { background: #f0fdfa; padding: 16px; border-radius: 12px; border: 1px solid #99f6e4; }
